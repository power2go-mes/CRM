...
The current authentication and role upgrade is consolidated in `supabase/setup.sql`. Run it against the existing CRM database in the Supabase SQL editor. It is incremental and must not be replaced with a fresh-schema script.

```bash
# Paste supabase/setup.sql into the target Supabase SQL editor and run it once.
...
└── setup.sql               # Consolidated database schema, functions, policies, grants, and seed data
```
