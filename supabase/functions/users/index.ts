...
async function updateSaleRole(
  ...
  reports_to_user_id: string | null | undefined,
...
async function _inviteUser(req: Request, currentUserSale: any) {
...
async function patchUser(req: Request, currentUserSale: any) {
  ...
  const isSuperAdmin =
    currentUserSale.role === "super_admin" || currentUserSale.administrator;

  // Users can update their own profile; only Super Admin can manage roles.
  if (!isSuperAdmin && currentUserSale.id !== sale.id) {
    return createErrorResponse(401, "Not Authorized");
  }
  ...
  // Only Super Admin can update role, hierarchy, and active status.
  if (!isSuperAdmin) {
  ...
  try {
    ...
    const sale = await updateSaleRole(
      data.user.id,
      role ?? (administrator ? "super_admin" : "bdo"),
      designation,
      reports_to_user_id,
      region_id,
      region,
