...
export const CRM = ({
  ...
  useEffect(() => {
    const existingConfig = store.getItem<ConfigurationContextValue>(
      CONFIGURATION_STORE_KEY,
    );
    if (
      !existingConfig ||
      existingConfig.title === "Atomic CRM" ||
      existingConfig.currency === "USD"
    ) {
      const targetTitle =
        existingConfig?.title === "Atomic CRM"
          ? title
      ...
      store.setItem(CONFIGURATION_STORE_KEY, {
        companySectors,
        currency:
          existingConfig?.currency === "USD" ? defaultCurrency : currency,
        dealCategories,
