...
export const createDataProvider = ({
  ...
  const dataProviderWithCustomMethod: CrmDataProvider = {
    ...
    async getList(resource: string, params: any) {
      if (resource === "users") {
        return baseDataProvider.getList("sales", params);
      }
        ...
        return {
          ...
          data: result.data.map((region) => {
            ...
            return {
              ...
              rsm_count: users.filter((sale) => sale.role === "rsm").length,
              ssm_count: users.filter((sale) => sale.role === "ssm").length,
              asm_count: users.filter((sale) => sale.role === "asm").length,
              bdo_count: users.filter((sale) => sale.role === "bdo").length,
            };
          }),
        };
      }
    ...
    [
      {
        ...
        beforeDelete: async (params) => {
          const linked = await dataProvider.getList<Sale>("sales", {
            filter: { region_id: params.id },
            pagination: { page: 1, perPage: 1 },
            sort: { field: "id", order: "ASC" },
          });
          if (linked.total) {
      ...
      } satisfies ResourceCallbacks<Region>,
      {
