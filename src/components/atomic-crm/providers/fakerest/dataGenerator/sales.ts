import { internet, name } from "faker/locale/en_US";

import type { RAFile, Sale } from "../../../types";
import { ROLE } from "../../commons/roles";
import type { Db } from "./types";

export const generateSales = (_: Db): Sale[] => {
  const randomSales = Array.from(Array(5).keys()).map((id) => {
    const first_name = name.firstName();
    const last_name = name.lastName();
    const email = internet.email(first_name, last_name);

    return {
      id: id + 1,
      user_id: `${id + 1}`,
      first_name,
      last_name,
      email,
      secondary_emails: [],
      password: "demo",
      administrator: false,
      role: ROLE.BDO,
      designation: "Business Development Officer",
      region_id: 1,
      reports_to_user_id: "7",
      disabled: false,
    };
  });
  return [
    {
      id: 0,
      user_id: "0",
      first_name: "Jane",
      last_name: "Doe",
      email: "janedoe@atomic.dev",
      secondary_emails: [],
      password: "demo",
      administrator: true,
      role: ROLE.SUPER_ADMIN,
      disabled: false,
      avatar: {
        src: "data:image/jpeg;base64,",
      } as RAFile,
    },
    { id: 6, user_id: "6", first_name: "South", last_name: "ASM", email: "asm.south@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.ASM, designation: "Area Sales Manager", region_id: 1, reports_to_user_id: "7", disabled: false },
    { id: 7, user_id: "7", first_name: "South", last_name: "SSM", email: "ssm.south@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.SSM, designation: "Senior Sales Manager", region_id: 1, reports_to_user_id: "8", disabled: false },
    { id: 8, user_id: "8", first_name: "South", last_name: "RSM", email: "rsm.south@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.RSM, designation: "Regional Sales Manager", region_id: 1, reports_to_user_id: null, disabled: false },
    { id: 9, user_id: "9", first_name: "Sales", last_name: "Head", email: "head.sales@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.HEAD_OF_SALES, designation: "Head of Sales", region_id: null, reports_to_user_id: null, disabled: false },
    { id: 10, user_id: "10", first_name: "North", last_name: "ASM", email: "asm.north@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.ASM, designation: "Area Sales Manager", region_id: 2, reports_to_user_id: "11", disabled: false },
    { id: 11, user_id: "11", first_name: "North", last_name: "SSM", email: "ssm.north@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.SSM, designation: "Senior Sales Manager", region_id: 2, reports_to_user_id: "12", disabled: false },
    { id: 12, user_id: "12", first_name: "North", last_name: "RSM", email: "rsm.north@finlonexa.test", secondary_emails: [], password: "demo", administrator: false, role: ROLE.RSM, designation: "Regional Sales Manager", region_id: 2, reports_to_user_id: null, disabled: false },
    ...randomSales,
  ];
};
