...
const getDataProviderWithCustomMethods = () => {
  ...
  return {
    ...
    async getList(resource: string, params: GetListParams) {
      ...
      if (resource === "regions") {
        return baseDataProvider.getList("region_stats", params);
      }
      if (resource === "leads") {
    ...
    async getOne(resource: string, params: any) {
      if (resource === "users") {
        return baseDataProvider.getOne("sales", params);
      }
      if (resource === "companies") {
    ...
    async salesCreate(body: SalesFormData) {
      ...
      if (!data || error) {
        ...
        throw Object.assign(
          new Error(errorDetails?.message || error?.message || "Failed to create the user"),
