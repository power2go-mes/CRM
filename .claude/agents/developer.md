...
- TypeScript types + test-only data fixtures where existing tests require them.
...
- **Self-verification in the browser (optional, before commit).** For behavior-visible changes, use the Playwright MCP against the Vite dev server started **inside your own worktree** (never `$REPO`). Configure `VITE_SUPABASE_URL` and `VITE_SB_PUBLISHABLE_KEY`, then run `npm run dev -- --port <PORT> --strictPort` in the background on `config.app.portBase` plus the TASK number. The app uses hash routing (`http://localhost:<PORT>/#/<route>`) and real Supabase data; use an approved test account and avoid writes to shared production data. There is no FakeRest or demo fallback. Prefer `browser_snapshot` over screenshots except for visual criteria. **Always tear down** the browser and server before stopping. This does not replace the required e2e spec.
