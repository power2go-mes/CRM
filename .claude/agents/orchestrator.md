...
- **Supabase smoke** (when the diff changes UI under `src/components/`): dispatch the quality-reviewer with `MODE: feature-smoke` (foreground, `run_in_background: false`) to drive the feature's key user flows via the Playwright MCP against the configured Supabase environment. It requires valid Vite Supabase environment variables and a suitable test account; if either is unavailable, report the smoke as blocked rather than falling back to mock data.
